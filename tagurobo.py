...
class RobotArmSDK:
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            self.sock.sendto(test_command.encode('utf-8'), (self.ip_address, self.port))
    ...
    def disconnect(self) -> None:
        ...
        if self.connected and self.sock:
            try:
                ...
                self.sock.sendto(disconnect_command.encode('utf-8'), (self.ip_address, self.port))
            except:
                pass  # 切断時のエラーは無視
    ...
    def _send_command_and_wait_response(self, command: str) -> Optional[str]:
        ...
        try:
            # コマンドを送信
            self.sock.sendto(command.encode('utf-8'), (self.ip_address, self.port))
    ...
    def set_joint_limits(self, joint_id: int, min_angle: float, max_angle: float) -> bool:
        """
        関節の角度制限を設定する
        ...
        if min_angle >= max_angle:
            print(f"エラー: 最小角度は最大角度より小さくする必要があります")
            return False
    ...
    def set_joint_angle(self, joint_id: int, angle: float, speed: float = 50.0) -> bool:
        """
        指定した関節の角度を設定する
    ...
    def set_all_joint_angles(self, angles: List[float], speed: float = 50.0) -> bool:
        """
        すべての関節の角度を同時に設定する
        ...
        if len(angles) != self.num_joints:
            print(f"エラー: 角度リストの長さが関節数と一致しません")
            return False
    ...
    def execute_trajectory(self, trajectory: List[List[float]], speed: float = 50.0) -> bool:
        """
