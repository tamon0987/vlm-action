...
class ESP32RobotTester:
    """ESP32ロボットアームのテストツール（UDP/HTTP両対応）"""
    
    def __init__(self, ip_address: str = "127.0.0.1", udp_port: int = 4210, 
                 http_port: int = 80, use_mock: bool = True):
        self.ip_address = ip_address
        self.udp_port = udp_port
        self.http_port = http_port
        self.use_mock = use_mock
        self.base_url = f"http://{ip_address}:{http_port}"
        
        # UDP用ソケット
        self.udp_sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        self.udp_sock.settimeout(2.0)
        
    def send_udp_command(self, command: str) -> Optional[str]:
        """UDPコマンドを送信して応答を取得"""
        try:
            ...
            print(f"[UDP] 送信: {command.strip()}")
            self.udp_sock.sendto(command.encode('utf-8'), (self.ip_address, self.udp_port))
            
            # 応答を待機
            data, addr = self.udp_sock.recvfrom(1024)
            response = data.decode('utf-8').strip()
            print(f"[UDP] 受信: {response}")
            return response
    ...
    def get_http_servos(self) -> Optional[List[Dict]]:
        """HTTPでサーボ状態を取得"""
        try:
            response = requests.get(f"{self.base_url}/servos", timeout=5)
    ...
    def set_http_servos(self, servo_data: List[Dict]) -> bool:
        ...
        try:
            ...
            response = requests.post(f"{self.base_url}/servos", 
                                   json=servo_data, 
                                   headers=headers,
                                   timeout=5)
            if response.status_code == 200:
    ...
    def stop_all_http(self) -> bool:
        """HTTPで全サーボを停止"""
        try:
            response = requests.post(f"{self.base_url}/stop_all", timeout=5)
...
def main():
    ...
    parser.add_argument('--ip', default='127.0.0.1', help='IPアドレス')
    ...
    # 実機の場合はIPアドレスを変更
    if not args.mock and args.ip == '127.0.0.1':
